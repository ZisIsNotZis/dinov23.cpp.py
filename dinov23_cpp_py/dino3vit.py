...
def run(x: np.ndarray, model=getenv('MODEL','dinov3-vits16-pretrain-lvd1689m-f16.gguf'), gpu=False) -> tuple[np.ndarray, np.ndarray]:
    ...
    if x.shape != SHAPE:
        ...
        initgraph(Y.normscale('norm').named('y'))
...
if __name__ == '__main__':
    ...
    for i, x, y in zip(argv, x, z):
        x.tofile(f'{i}dino3vit.{','.join(map(str, x.shape))}f')
        y.tofile(f'{i}dino3vit.{','.join(map(str, y.shape))}f')
    if getenv('VIS', ''):
        ...
        for i, z in zip(argv, z.astype('B')):
            fromarray(z).resize((z.shape[1] * PATCHSZ, z.shape[0] * PATCHSZ), Resampling.LANCZOS).save(f'{i}dino3vit.vis', 'jpeg')
