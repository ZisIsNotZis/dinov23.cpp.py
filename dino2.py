...
def run(x: np.ndarray, model='dinov2-with-registers-small-imagenet1k-1-layer-f16.gguf', gpu=False) -> tuple[np.ndarray, np.ndarray]:
    ...
    if x.shape != SHAPE:
        ...
        Y = Y.rcat(Tensor('embeddings.cls_token'), 1).add_(EMB).rcat(Tensor('embeddings.register_tokens'), 1)
        for i in range(layer):
            Y = Y.norm(f'encoder.layer.{i}.norm1').at(f'encoder.layer.{i}.attention').mul_(Tensor(f'encoder.layer.{i}.layer_scale1.lambda1')).add_(Y)
            Y = (Tensor.mlp if layer != 40 else Tensor.swiglu)(Y.norm(f'encoder.layer.{i}.norm2'), f'encoder.layer.{i}.mlp').mul_(Tensor(f'encoder.layer.{i}.layer_scale2.lambda1')).add_(Y)
        initgraph(Y.norm_('layernorm').named('y'))
